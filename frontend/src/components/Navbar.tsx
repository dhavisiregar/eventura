        ...
        <nav className="hidden items-center gap-6 md:flex">
          {links.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className={`text-sm font-medium ${pathname === l.href ? "text-indigo-600" : "text-slate-600 hover:text-slate-900"}`}
            >
              {l.label}
            </Link>
          ))}
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          {!loading && user?.role === "customer" && (
            <>
              ...
              <Link href="/account" className="flex items-center gap-2 text-sm font-medium text-slate-700">
      ...
      {open && (
        <div className="border-t border-slate-200 bg-white px-4 py-3 md:hidden">
          <div className="flex flex-col gap-2">
            {links.map((l) => (
              <Link key={l.href} href={l.href} className="py-2 text-sm font-medium text-slate-700" onClick={() => setOpen(false)}>
                {l.label}
              </Link>
            ))}
            {!loading && !user && (
