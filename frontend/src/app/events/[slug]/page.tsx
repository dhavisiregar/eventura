...
export default function EventDetailPage() {
  ...
  if (loading) {
    return <div className="mx-auto max-w-5xl px-4 py-16 text-center text-slate-500 dark:text-slate-400">Loading event…</div>;
  }
  if (notFound || !event) {
    return (
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="aspect-[21/9] w-full overflow-hidden rounded-2xl bg-linear-to-br from-indigo-100 to-slate-100 dark:from-indigo-500/10 dark:to-slate-800">
        {banner && (
      ...
      <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <h1 className="mt-3 text-2xl font-bold text-slate-900 sm:text-3xl dark:text-slate-100">{event.title}</h1>
          ...
          <Card className="mt-6 p-5">
            <h2 className="mb-2 text-sm font-semibold text-slate-900 dark:text-slate-100">About this event</h2>
            <p className="whitespace-pre-line text-sm leading-relaxed text-slate-600 dark:text-slate-400">{event.description}</p>
          </Card>

          <div className="mt-8">
            <h2 className="mb-4 text-sm font-semibold text-slate-900 dark:text-slate-100">Reviews ({reviewCount})</h2>
            {reviews.length === 0 ? (
                ...
                {reviews.map((r) => (
                  <Card key={r.id} className="p-4">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-slate-900 dark:text-slate-100">{r.user?.name ?? "Anonymous"}</span>
