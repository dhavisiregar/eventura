    ...
    <div>
      ...
      {loading || !stats ? (
        ...
        <>
            ...
            {tiles.map((t) => (
              <Card key={t.label} className="p-4">
                <div className="flex items-center gap-2 text-xs font-medium text-slate-500 dark:text-slate-400">
                  <t.icon className="h-4 w-4 text-indigo-500 dark:text-indigo-400" /> {t.label}
                </div>
                <p className="mt-2 text-xl font-bold text-slate-900 dark:text-slate-100">{t.value}</p>
              </Card>
            ))}
          </div>

          <Card className="mt-6 p-5">
            <h2 className="mb-4 text-sm font-semibold text-slate-900 dark:text-slate-100">Revenue over time</h2>
              ...
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.series} margin={{ left: 8, right: 8 }}>
                    ...
                    />
                    <Bar dataKey="revenue" fill={isDark ? "#818cf8" : "#6366f1"} radius={[6, 6, 0, 0]} maxBarSize={40} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
          ...
          <Card className="mt-6 p-5">
            <h2 className="mb-4 text-sm font-semibold text-slate-900 dark:text-slate-100">Top events</h2>
            {stats.top_events.length === 0 ? (
              <p className="text-sm text-slate-500 dark:text-slate-400">No sales in this range yet.</p>
            ) : (
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {stats.top_events.map((e) => (
                      <tr key={e.title} className="border-b border-slate-50 last:border-0 dark:border-slate-800/60">
                        <td className="py-2.5 text-slate-800 dark:text-slate-200">{e.title}</td>
                        <td className="py-2.5 text-slate-600 dark:text-slate-400">{e.tickets_sold}</td>
                        <td className="py-2.5 font-medium text-slate-900 dark:text-slate-100">{formatIDR(e.revenue)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
